...
local function is_cursor_in_match(col, match)
    return col + 1 >= match.start_pos and col + 1 <= match.end_pos
end
                    ...
                    for _, match in ipairs(matches) do
                        if is_cursor_in_match(col, match) then
                            ...
                            if not best_match or (M.config.find_smallest_match and size < best_size) then
                                local next_index = options.reverse and (i == 1 and #def or i - 1)
                                    or (i == #def and 1 or i + 1)
                                best_match = match
                                best_pattern = value
...
function M.setup()
    ...
    vim.api.nvim_create_user_command("SwitchReverse", function()
        M.switch { reverse = true }
    end, {})

    if M.config.mapping ~= "" then
        vim.keymap.set("n", M.config.mapping, function()
            M.switch()
        end, { silent = true, desc = "Switch" })
    end

    if M.config.reverse_mapping ~= "" then
        vim.keymap.set("n", M.config.reverse_mapping, function()
            M.switch { reverse = true }
        end, { silent = true, desc = "Switch Reverse" })
    end
end
