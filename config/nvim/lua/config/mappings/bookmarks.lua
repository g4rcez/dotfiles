...
function Bookmarks.add_bookmark(description)
...
function Bookmarks.remove_bookmark(bookmark_or_index)
    local current_bookmarks = get_current_bookmarks()
...
function Bookmarks.clear_bookmarks()
    local project_key = get_project_key()
    bookmarks[project_key] = {}
    save_bookmarks()
...
function Bookmarks.toggle_bookmark(description)
    local buf = vim.api.nvim_get_current_buf()
    local file = vim.api.nvim_buf_get_name(buf)
    ...
    -- Add new bookmark
    Bookmarks.add_bookmark(description)
end

function Bookmarks.jump_to_bookmark(bookmark)
    ...
    require("snacks").picker.pick({
        ...
        confirm = function(picker, item)
            ...
            if item and item.bookmark then
                Bookmarks.jump_to_bookmark(item.bookmark)
            end
        end,
        actions = {
            delete_bookmark = function(picker, item)
                if item and item.bookmark then
                    Bookmarks.remove_bookmark(item.idx)
...
function Bookmarks.toggle()
    ...
    local line_content = vim.api.nvim_buf_get_lines(buf, cursor[1] - 1, cursor[1], false)[1] or ""
    local description = line_content:match("^%s*(.-)%s*$") -- Trim whitespace
    Bookmarks.toggle_bookmark(description)
end
...
function Bookmarks.clear()
    Bookmarks.clear_bookmarks()
    vim.notify("All bookmarks cleared", vim.log.levels.INFO)
end
...
function Bookmarks.get_bookmarks()
