import { css, dotbot, ENV, promiseSequence, SUCCESS_OUTPUT_STYLE, trim } from "@dotfiles/core";
...
const Vscode = {
    ...
    saveExtension: async (pathTo: string, missing: string[]) => {
        ...
        const content = await dotbot.cat(pathTo);
        const unique = new Set<string>(content.split("\n").concat(missing));
        const newContent = Vscode.sanitizeExtensions(Array.from(unique));
        await dotbot.write(pathTo, newContent.join("\n"));
    ...
    extensions: async (path: string | undefined, extensions: string[] = []): Promise<string[]> => {
        ...
        if (path) {
            const content = await dotbot.cat(path);
    ...
    link: async (entryDir: string, copyTo: string) => {
        const dirExists = await dotbot.exists(copyTo);
        if (!dirExists) {
            await Deno.mkdir(copyTo, { recursive: true });
        }
        for await (const dirEntry of Deno.readDir(entryDir)) {
            await dotbot.link(
                dotbot.join(entryDir, dirEntry.name),
                dotbot.join(copyTo, dirEntry.name),
...
export const vscodePlugin: DotbotPlugin<VscodePluginArgs> = (args) => async (plugin) => {
