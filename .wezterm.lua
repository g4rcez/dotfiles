...
local keys = {
  ...
  { key = 'o', mods = 'CTRL|SHIFT', action = wt.action.SplitHorizontal { domain = 'CurrentPaneDomain' }, },
  { key = 'p', mods = 'CTRL|SHIFT', action = wt.action.SplitVertical { domain = 'CurrentPaneDomain' }, },
...
local function launchItem(name) return {
...
return {
  font = wt.font 'FiraCode Nerd Font Mono',
  ...
  quick_select_patterns = { '[0-9a-f]+' },
  selection_word_boundary = " \t\n[]\"'`(),.;:",
  unix_domains = domains,
  use_fancy_tab_bar = false,
  window_background_opacity = 0.75,
  window_decorations = "NONE",
  window_padding = { left = 1, right = 0, top = 0, bottom = 1, },
}
