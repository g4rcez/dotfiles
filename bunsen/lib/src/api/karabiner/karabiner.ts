...
const utils = {
  keys: Object.keys,
  replaceHomedir: (str: string, symbol: string = '~') => str.replace(symbol, HOME),
}

export const createLeaderLayers = (config: Config): KarabinerModConfiguration => {
          ...
          manipulators: [
            {
              ...
              to: subMotion.to!.concat(karabiner.vim.off(key, false), karabiner.notify()),
              type: 'basic',
            },
            {
              conditions: [
...
type FrontMostApplicationCondition = {
  type: 'frontmost_application_if' | 'frontmost_application_unless' | 'frontmost_application_if'
...
const createHyperSubLayer = (
  ...
  const cmds = utils.keys(commands) as KeyCode[]
...
const createHyperSubLayers = (modKeys: SubLayers): KarabinerModConfiguration => {
  const allSubLayerVariables = utils.keys(modKeys).map((x) => createSubLayerName(x as KeyCode))
  ...
  return { layers: modSubLayers, keys: Object.keys(modKeys), whichKey: whichKeyMap }
}
...
export const createKarabinerConfig = async (params: {
  rules: KarabinerRule[]
  whichKey: WhichKey[]
  whichKeyFile: string
  configFile: string
  ...
  const args = {
    rules: params.rules,
    whichKey: params.whichKey,
    configFile: params.configFile,
    whichKeyFile: params.whichKeyFile,
  }
  params.whichKeyFile = utils.replaceHomedir(params.whichKeyFile)
  params.configFile = utils.replaceHomedir(params.configFile)
  const dirname = path.dirname(params.whichKeyFile)
  const dirExist = fs.existsSync(dirname)
  if (!dirExist) {
    fs.mkdirSync(dirname, { recursive: true })
  }
  fs.writeFileSync(params.whichKeyFile, JSON.stringify({ items: args.whichKey }), 'utf-8')
  fs.writeFileSync(
    params.configFile,
    JSON.stringify(
      {
  ...
  console.log(
    `%c[karabiner] %c${utils.replaceHomedir(params.configFile)} was created`,
    'color: purple',
    ''
  )
  console.log(
    `%c[karabiner] %cwhichkey: ${utils.replaceHomedir(params.whichKeyFile)} was created`,
    'color: purple',
    ''
  )
}
...
type KarabinerModConfiguration = {
  keys: string[]
  whichKey: WhichKey[]
...
export class Karabiner {
  public configPath = '~/.config/karabiner/karabiner.json'
  public whichKeyPath = '~/.config/karabiner/karabiner-whichkey.json'

  public global: KarabinerGlobalConfig = {
    ask_for_confirmation_before_quitting: true,
    check_for_updates_on_startup: true,
    show_in_menu_bar: true,
    show_profile_name_in_menu_bar: false,
  ...
  public profiles: KarabinerProfileConfig[] = []

  public profile(
    name: string,
    hyperKeyRule: KarabinerRule,
    rules: KarabinerModConfiguration[],
    params?: KarabinerProfileConfig['complex_modifications']['parameters']
  ) {
    this.profiles.push({
      name: name,
      whichKey: rules.flatMap((x) => x.whichKey),
      complex_modifications: {
        rules: [hyperKeyRule, ...rules.flatMap((x) => x.layers)],
        parameters: {
          'basic.simultaneous_threshold_milliseconds':
            params?.['basic.simultaneous_threshold_milliseconds'] || 50,
          'basic.to_delayed_action_delay_milliseconds':
            params?.['basic.to_delayed_action_delay_milliseconds'] || 250,
          'basic.to_if_alone_timeout_milliseconds':
            params?.['basic.to_if_alone_timeout_milliseconds'] || 250,
          'basic.to_if_held_down_threshold_milliseconds':
            params?.['basic.to_if_held_down_threshold_milliseconds'] || 250,
          'mouse_motion_to_scroll.speed': params?.['mouse_motion_to_scroll.speed'] || 100,
        },
      },
    })
  }
  public compile(whichKey: WhichKey[], ...t: Array<KarabinerRule | KarabinerRule[]>) {
    return {
      whichKey,
  ...
  public browser(
    ...
    return {
      description: description || `Open ${this.BROWSER} ${profile}`,
      to: [
        {
          shell_command: `open ${append} -a '${this.BROWSER}.app' --args --profile-directory='${profile}'`,
        },
      ],
    }
  }
  ...
  public aerospace = aerospace
  public rectangle = rectangle
  public appInstance = appInstance
  public replaceWhichKeys = replaceWhichKeys
  public createLeaderLayers = createLeaderLayers
  public createSubLayerName = createSubLayerName
  public createLeaderDisable = createLeaderDisable
  public createHyperSubLayers = createHyperSubLayers
}
