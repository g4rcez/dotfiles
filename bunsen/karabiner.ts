import { Karabiner, type Manipulator } from "@g4rcez/bunsen";
...
const keys = [
	...
	karabiner.createLeaderLayers({
		...
		s: {
			...
			s: karabiner.shell(
				"$HOME/dotfiles/bin/proofread-selection",
...
const disableLeaderKeys = keys.flatMap((key): Manipulator[] => {
	return key.keys.flatMap((k) => karabiner.createLeaderDisable(k, false));
});
